...
model_dir = sprintf('%sfmri/2ndLevel/main_withoutheadmoved/',output_path);
...
% Remove participants with excessive head motion and participant who failed
% too many trials 

forremoval_table = readtable(sprintf('%s/headmoved_ccids.csv',output_path));
forremoval_array = table2cell(forremoval_table);
forremoval_array(height(forremoval_array)+1,1) = {520055};

for i = 1:height(forremoval_array)
    IndexC = strfind(all_contrasts, num2str(forremoval_array{i,1}));
    Index = find(not(cellfun('isempty',IndexC))); 
    all_contrasts(Index,:)='';

    ccid_scan = ccid_scan(ccid_scan~=forremoval_array{i,1});
end
